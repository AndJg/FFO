...
if(isset($_POST['hashtag'])){
    ...
    $hashtag = $getFromUser->inputCheck($_POST['hashtag']);
    $mention = $getFromUser->inputCheck($_POST['hashtag']);
    if(substr($hashtag,0,1) === '#'){
    
        $trend = str_replace('#', '', $hashtag);
        $trends = $getFromRec->getTrendByHash($trend);    
    ...
    if(substr($mention,0,1) === '@'){
        ...
        $mentions = $getFromRec->getMention($mention);    
    
        foreach ($mentions as $mention) {
         
            echo'<li><div class="nav-right-down-inner">
            <div class="nav-right-down-left">
                <span><img src="'.BASE_URL.$mention->profilePic.'"></span>
            </div>
