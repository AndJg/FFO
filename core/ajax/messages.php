...
if(isset($_POST['sendMessage']) && !empty($_POST['sendMessage'])){
	...
	$user_id = $_SESSION['user_id'];
	$message = $getFromUser->inputCheck($_POST['sendMessage']);
	$get_id = $_POST['get_id'];
	if(!empty($message)){

		$getFromUser->create('messages', array('messageTo' => $get_id, 'messageFrom' => $user_id, 'message' => $message, 'messageOn' => date('Y-m-d H:i:s')));
...
if(isset($_POST['showChatMessage']) && !empty($_POST['showChatMessage'])){
	...
	$messages = $getFromMessage->getMessages($messageFrom, $user_id);
}
...
if(isset($_POST['showMessage']) && !empty($_POST['showMessage'])){
	...
	$getFromMessage->messagesViewed($user_id);
    ?>
<div class="popup-message-wrap">
<input id="popup-message-recommend" type="checkbox" checked="unchecked"/>
<div class="wrap2">
<div class="message-send">
	<div class="message-header">
		...
		<div class="message-h-right">
			<label for="popup-message-recommend" ><i class="fa fa-times" aria-hidden="true"></i></label>
		</div>
	</div>
			...
			<div class="people-message" data-user="<?php echo $message->user_id; ?>">
				<div class="people-inner">
					<div class="people-img">
						<img src="<?php echo BASE_URL.$message->profilePic; ?>"/>
					</div>
					<div class="name-right2">
						<span><a href="#"><?php echo $message->screenName; ?></a></span><span>@U<?php echo $message->username; ?></span>
					</div>
...
<div class="message-send2">
	<div class="message-header2">
		...
		<div class="message-h-cen">
			...
			<img src="<?php echo BASE_URL.$user->profilePic; ?>"/><h4>Messages</h4>
			</div>
		</div>
		<div class="message-h-right">
